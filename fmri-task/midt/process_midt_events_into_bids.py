...
def extract_subject_session_run_id(bids_file):
    ...
    match = re.match(r'sub-(\d+)_ses-(\d+)_task-midt_run-(\d+)_eprime\.txt', filename)
    if match:
...
def process_midt_subjects(subject_sessions, dry_run):
    ...
    if not subject_sessions:
        subject_sessions = glob.glob(os.path.join(base_dir, "**", "sub-*midt*eprime.txt"), recursive=True)
    ...
    for subject_logfile in subject_sessions:
        ...
        cmd = ['python', '/home/spinney/project/spinney/neuroventure/fmri-task/midt/create_fmri_midt_eventfiles.py',
...
if __name__ == "__main__":
    ...
    process_midt_subjects(args.subject_sessions, args.dry_run)
