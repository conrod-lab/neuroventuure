...
log_file="${SCRATCH}/verify_fmri_stop_runs_log.csv"  # Replace with the actual path for the log file
error_log_file="${SCRATCH}/verify_fmri_stop_runs_error_log.txt"  # Replace with the actual path for the error log file
    ...
    if [ -d "$subject_dir" ]; then
        ...
        for session_dir in "$subject_dir"/ses-*; do
            if [ -d "$session_dir" ]; then
                ...
                for fmri_task_file in "$session_dir"/func/*stop*_bold.nii.gz; do
                    if [ -f "$fmri_task_file" ]; then
                        ...
                        eprime_raw_file=$(find "$eprime_dir/V$session_number/NV_$subject_number" -type f -iname "*STOP_fMRI*.txt"  | head -n 1)
